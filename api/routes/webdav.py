...
from application.auth.dependencies import User, authenticate_user_db
...
async def _ensure_webdav_enabled() -> None:
    enabled = await config_service.get(_WEBDAV_ENABLED_KEY, "1")
...
async def _get_basic_user(request: Request) -> User:
    ...
    if scheme_lower == "basic":
        ...
        user_or_false: Optional[User] = await authenticate_user_db(
            username, password
        )
...
async def options_root(
    path: str = "", _enabled: None = Depends(_ensure_webdav_enabled)
):
...
async def propfind(
    ...
    try:
        ...
        responses.append(
            _build_prop_response(full_path, name, is_dir, size, mtime, ctype)
        )
    ...
    if depth in ("1", "infinity"):
        try:
            ...
            for ent in listing["items"]:
                ...
                mtime = (
                    int(ent.get("mtime", 0)) if ent.get("mtime") is not None else None
                )
                ctype = (
                    None
                    if is_dir
                ...
                responses.append(
                    _build_prop_response(child_path, name, is_dir, size, mtime, ctype)
                )
