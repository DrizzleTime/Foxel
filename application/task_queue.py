...
class TaskQueueService:
    ...
    async def update_progress(
        self, task_id: str, progress: TaskProgress | Dict[str, Any]
    ):
    ...
    async def _execute_task(self, task: Task):
        ...
        await logging_service.info(
            "task_queue",
            f"Task {task.name} ({task.id}) started",
        ...
        try:
            if task.name == "process_file":
                ...
                result = await virtual_fs_service.process_file(
                    path=params["path"],
                ...
                if task.name == "automation_task":
                    ...
                    if task_id is None:
                        raise ValueError("Automation task missing task_id")
                    rule = await automation_service.get_task(int(task_id))
                    if not rule:
                        raise ValueError(f"Automation task {task_id} not found")
                    if not rule.enabled:
                        task.result = "Automation rule disabled; skipped"
                        task.status = TaskStatus.SUCCESS
                        return
                    processor_type = rule.processor_type
                    config = rule.processor_config or {}
                    save_to = rule.processor_config.get("save_to") if rule else None
                    path = params.get("path")

                if not path:
                    raise ValueError("Missing path for processor task")

                processor = processor_service.get_processor(processor_type)
                if not processor:
                    raise ValueError(f"Processor {processor_type} not found")

                file_content = await virtual_fs_service.read_file(path)
                result = await processor.process(file_content, path, config)

                if save_to and getattr(processor, "produces_file", False):
                    await virtual_fs_service.write_file(save_to, result)
                task.result = (
                    "Automation task completed"
                    if rule
                    else f"Processor task {processor_type} completed"
                )
            elif task.name == "offline_http_download":
                from application.offline_download.service import run_http_download

                result_path = await run_http_download(task)
                task.result = {"path": result_path}
            elif task.name == "cross_mount_transfer":
                result = await virtual_fs_service.run_cross_mount_transfer_task(task)
                task.result = result
            ...
            await logging_service.info(
                "task_queue",
                f"Task {task.name} ({task.id}) succeeded",
                {"task_id": task.id, "name": task.name},
            )
    ...
    def _is_processor_task(self, task_name: str) -> bool:
        try:
            ...
            return processor_service.get_processor(task_name) is not None
        except Exception:
            return False
    ...
    async def _ensure_worker_count(self):
        ...
        if current < self._concurrency:
            ...
            await logging_service.info(
                "task_queue",
                "Task workers adjusted",
                {
                    "active_workers": len(self._worker_tasks),
                    "target": self._concurrency,
                },
            )
    ...
    async def _worker_loop(self, worker_id: int):
        ...
        await logging_service.info("task_queue", f"Worker {worker_id} started")
        try:
            while True:
                ...
                if job is _SENTINEL:
                    ...
                    break
                try:
                    await self._execute_task(job)  # type: ignore[arg-type]
                except Exception as e:  # noqa: BLE001
                    await logging_service.error(
                        "task_queue",
                        f"Error executing task {getattr(job, 'id', '?')}: {e}",
                        {"task_id": getattr(job, 'id', None), "name": getattr(job, 'name', None)},
                    )
                finally:
                    self._queue.task_done()
        finally:
            ...
            await logging_service.info("task_queue", f"Worker {worker_id} stopped")

    async def start_worker(self, concurrency: int | None = None):
        if concurrency is None:
            stored_value = await config_service.get(
                "TASK_QUEUE_CONCURRENCY", self._concurrency
            )
            try:
    ...
    async def stop_worker(self):
        ...
        await logging_service.info("task_queue", "Task workers have been stopped.")
