...
from application.s3.dependencies import s3_mapping_service
from application.s3.use_cases import S3Settings, etag, meta_headers, normalize_ws, now_iso, sign
...
def _s3_error(
    code: str, message: str, resource: str = "", status: int = 400
) -> Response:
    req_id, headers = meta_headers()
...
def _s3_error(
    ...
    return Response(
        content=xml, status_code=status, media_type="application/xml", headers=headers
    )


async def _ensure_enabled() -> Optional[Response]:
    enabled = await s3_mapping_service.ensure_enabled()
    if not enabled:
...
async def _get_settings() -> Tuple[Optional[S3Settings], Optional[Response]]:
    settings, error = await s3_mapping_service.get_settings()
    if error:
        return None, _s3_error("InvalidAccessKeyId", error, status=403)
...
async def _authorize_sigv4(
    request: Request, settings: S3Settings
) -> Optional[Response]:
    ...
    if not auth.startswith(scheme + " "):
        return _s3_error(
            "InvalidRequest", "Signature Version 4 is required", status=400
        )
    ...
    if not credential or not signed_headers or not signature:
        return _s3_error(
            "InvalidRequest", "Authorization header is malformed", status=400
        )
    ...
    if region != settings["region"]:
        return _s3_error(
            "AuthorizationHeaderMalformed", f"Region '{region}' is invalid", status=400
        )
    ...
    if not amz_date or not amz_date.startswith(datestamp):
        return _s3_error(
            ...
            "x-amz-date does not match credential scope",
            status=400,
        )
    ...
    if not payload_hash:
        return _s3_error(
            "AuthorizationHeaderMalformed", "Missing x-amz-content-sha256", status=400
        )
    if payload_hash.upper().startswith("STREAMING-AWS4-HMAC-SHA256"):
        return _s3_error(
            "NotImplemented", "Chunked uploads are not supported", status=400
        )

    signed_header_names = [
        h.strip().lower() for h in signed_headers.split(";") if h.strip()
    ]
    ...
    for name in signed_header_names:
        ...
        if value is None:
            return _s3_error(
                "AuthorizationHeaderMalformed",
                f"Signed header '{name}' missing",
                status=400,
            )
        canonical_headers.append(f"{name}:{normalize_ws(value)}\n")
    ...
    k_date = sign(("AWS4" + settings["secret_key"]).encode("utf-8"), datestamp)
...
async def _collect_objects(
    path: str, key_prefix: str, recursive: bool, collect_prefixes: bool
...
) -> Tuple[List[Tuple[str, Dict]], List[str], bool, Optional[str]]:
    combined = [(key, data, True) for key, data in entries] + [
        (prefix, None, False) for prefix in prefixes
    ]
...
def _format_contents(entries: List[Tuple[str, Dict]]) -> str:
    ...
    for key, meta in entries:
        ...
        last_modified = dt.datetime.utcfromtimestamp(
            mtime_val or dt.datetime.utcnow().timestamp()
        ).strftime("%Y-%m-%dT%H:%M:%S.000Z")
        etag_value = etag(key, size, mtime_val)
        blocks.append(
            f"<Contents><Key>{key}</Key><LastModified>{last_modified}</LastModified><ETag>{etag_value}</ETag><Size>{size}</Size><StorageClass>STANDARD</StorageClass></Contents>"
        )
    return "".join(blocks)
...
async def list_buckets(request: Request):
    ...
    xml = (
        ...
        f"<Buckets><Bucket><Name>{settings['bucket']}</Name><CreationDate>{now_iso()}</CreationDate></Bucket></Buckets>"
...
async def list_objects(request: Request, bucket: str):
    ...
    if prefix and not prefix.endswith("/"):
        try:
            ...
            if exc.status_code not in (400, 404):
                raise
        if files:
            contents, next_prefixes, is_truncated, next_token = _apply_pagination(
                files, [], max_keys, start_after, continuation
            )
            xml = _build_list_result(
                bucket,
                prefix,
                delimiter,
                contents,
    ...
    files, prefixes = await _collect_objects(
        virtual_dir, dir_prefix, recursive, bool(delimiter)
    )

    contents, next_prefixes, is_truncated, next_token = _apply_pagination(
        files, prefixes if delimiter else [], max_keys, start_after, continuation
    )
    return _build_list_result(
        ...
        delimiter,
        contents,
        next_prefixes if delimiter else [],
        max_keys,
        is_truncated,
        continuation,
        next_token,
...
def _build_list_result(
    ...
    body = [
        f'<?xml version="1.0" encoding="UTF-8"?>',
        f'<ListBucketResult xmlns="{_XML_NS}">',
    ]
...
async def _ensure_bucket_and_auth(
    request: Request, bucket: str
) -> Tuple[Optional[S3Settings], Optional[Response]]:
    if (resp := await _ensure_enabled()) is not None:
        return None, resp
    settings, err = await _get_settings()
    if err:
...
async def _stat_object(
    settings: S3Settings, key: str
    ...
    try:
        ...
        if info.get("is_dir"):
            return None, _s3_error(
                ...
                _resource_path(settings["bucket"], key),
                status=404,
            )
        return info, None
    except HTTPException as exc:
...
async def put_object(request: Request, bucket: str, object_path: str):
    ...
    await write_file_stream(
        _virtual_path(settings, key), request.stream(), overwrite=True
    )
    meta, err = await _stat_object(settings, key)
    if err:
        return err
