...
async def get_or_create_thumb(adapter, adapter_id: int, root: str, rel: str, w: int, h: int, fit: str = "cover"):
    ...
    key = _cache_key(adapter_id, rel, size, int(stat.get("mtime", 0)), w, h, fit)
    ...
    if callable(get_thumb_impl):
        ...
        if native_thumb_bytes:
            try:
                ...
                print(f"Failed to convert native thumbnail to WebP: {e}, falling back.")
                thumb_bytes, mime = None, None
    ...
    if not thumb_bytes:
        if is_video:
            ...
            try:
                ...
                raise HTTPException(500, detail=f"Video thumbnail generation failed: {e}")
        else:
    ...
    raise HTTPException(500, detail="Failed to generate thumbnail by any means")
