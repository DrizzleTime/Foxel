...
from application.task_queue import Task, TaskProgress, task_queue_service
...
async def _path_exists(full_path: str) -> bool:
    try:
        await virtual_fs_service.stat_file(full_path)
        return True
...
async def run_http_download(task: Task):
    ...
    async def report_transfer(delta: int):
        ...
        percent = (
            min(100.0, round(bytes_done_transfer / file_size * 100, 2))
            if file_size
            else None
        )
        await task_queue_service.update_progress(
    ...
    await virtual_fs_service.write_file_stream(final_path, chunk_iter())
    ...
    await task_queue_service.update_meta(
        task.id, {"final_path": final_path, "filename": resolved_name}
    )
    ...
    try:
        ...
        await logging_service.info(
            "offline_download", f"Temp cleanup failed for task {task.id}"
        )

    return final_path


__all__ = ["run_http_download"]
