...
from application.storage.dependencies import mount_resolver
from application.task_queue import task_queue_service
from application.thumbnail import is_image_filename, is_raw_filename, is_video_filename
from domain.storage.entities import StorageMount
from domain.storage.exceptions import StorageGatewayNotReady, StorageMountNotFound
...
if TYPE_CHECKING:
    from application.task_queue import Task
...
async def resolve_adapter_by_path(path: str) -> Tuple[StorageMount, str]:
    try:
        mount, _, _, rel = await mount_resolver.resolve(path)
    except StorageMountNotFound as exc:
        raise HTTPException(404, detail="No storage adapter for path") from exc
    except StorageGatewayNotReady as exc:
        raise HTTPException(
            404,
            detail=f"Adapter instance for ID {exc.mount_id} not found or failed to load.",
        ) from exc
    return mount, rel
...
async def resolve_adapter_and_rel(path: str):
    ...
    try:
        mount, adapter_instance, effective_root, rel = await mount_resolver.resolve(
            path
        )
    except StorageMountNotFound as exc:
        raise HTTPException(404, detail="No storage adapter for path") from exc
...
async def list_virtual_dir(
    ...
    child_mount_entries: List[str] = await mount_resolver.list_child_mount_entries(norm)
    ...
    def annotate_entry(entry: Dict) -> None:
        if not entry.get("is_dir"):
            name = entry.get("name", "")
            entry["has_thumbnail"] = bool(
                is_image_filename(name) or is_video_filename(name)
            )
        else:
    ...
    try:
        (
            ...
            effective_root,
            rel,
        ) = await resolve_adapter_and_rel(norm)
...
async def write_file(path: str, data: bytes):
    ...
    await automation_service.trigger(event="file_written", path=path)
...
async def write_file_stream(
    path: str, data_iter: AsyncIterator[bytes], overwrite: bool = True
):
    adapter_instance, _, root, rel = await resolve_adapter_and_rel(path)
    if rel.endswith("/"):
        raise HTTPException(400, detail="Invalid file path")
...
async def make_dir(path: str):
    ...
    await logging_service.action(
        "virtual_fs", f"Created directory {path}", details={"path": path}
    )
...
async def delete_path(path: str):
    ...
    await automation_service.trigger(event="file_deleted", path=path)
    await logging_service.action("virtual_fs", f"Deleted {path}", details={"path": path})
...
async def move_path(
    ...
    if dst_exists and overwrite:
        try:
            ...
            raise HTTPException(500, detail=f"Pre-delete failed before overwrite: {e}")
    ...
    await logging_service.action("virtual_fs", f"Moved {src} to {dst}", details=debug_info)
    return debug_info if return_debug else None


async def rename_path(
    src: str, dst: str, overwrite: bool = False, return_debug: bool = True
):
    adapter_s, adapter_model_s, root_s, rel_s = await resolve_adapter_and_rel(src)
    adapter_d, adapter_model_d, root_d, rel_d = await resolve_adapter_and_rel(dst)
    debug_info = {
    ...
    await logging_service.action("virtual_fs", f"Renamed {src} to {dst}", details=debug_info)
    return debug_info if return_debug else None
        ...
        try:
            ...
            try:
                ...
                with rawpy.imread(io.BytesIO(raw_data)) as raw:
                    ...
                    if thumb is not None and thumb.format in [
...
async def _gather_vector_index(full_path: str, limit: int = 20):
    ...
    try:
        raw_results = await vector_db.search_by_path(
            "vector_collection", full_path, max(limit * 2, 20)
        )
    except Exception:
        return None
        ...
        for bucket in buckets:
            ...
            for record in bucket:
                ...
                entry = {
                    ...
                    if entity.get("chunk_id") is not None
                    else None,
...
async def stat_file(path: str):
    ...
    if isinstance(info, dict):
        ...
        try:
            ...
            is_dir = False
        rel_name = (
            rel.rstrip("/").split("/")[-1] if rel else path.rstrip("/").split("/")[-1]
        )
        name_hint = str(info.get("name") or rel_name or "")
        info["has_thumbnail"] = bool(
            not is_dir
            and (is_image_filename(name_hint) or is_video_filename(name_hint))
        )
        if not is_dir:
...
async def copy_path(
    ...
    try:
        ...
        await logging_service.action("virtual_fs", f"Copied {src} to {dst}", details=debug_info)
    return debug_info if return_debug else None


async def _enqueue_cross_mount_transfer(
    operation: str, src: str, dst: str, overwrite: bool
) -> Dict[str, Any]:
    ...
    if adapter_model_s.id == adapter_model_d.id:
        raise HTTPException(
            400, detail="Cross-adapter transfer requested but adapters are identical"
        )
...
async def run_cross_mount_transfer_task(task: "Task") -> Dict[str, Any]:
    ...
    if adapter_model_s.id == adapter_model_d.id:
        if operation == "move":
            await move_path(
                src, dst, overwrite=overwrite, return_debug=False, allow_cross=False
            )
        else:
            await copy_path(
                src, dst, overwrite=overwrite, return_debug=False, allow_cross=False
        ...
        while stack:
            ...
            while True:
                entries, total = await list_dir(
                    root_s, current_rel, page, page_size, "name", "asc"
                )
    ...
    try:
        for job in files_to_transfer:
            ...
            percent = None
            total_for_percent = (
                total_dynamic_bytes if total_dynamic_bytes else bytes_downloaded
            )
            if total_for_percent:
                percent = min(
                    100.0, round(bytes_downloaded / total_for_percent * 100, 2)
                )
            await task_queue_service.update_progress(
                task.id,
                {
        ...
        for dir_rel in sorted(
            {d for d in dirs_to_create if d}, key=lambda x: x.count("/")
        ):
        ...
        for job in files_to_transfer:
            ...
            await write_file_stream(
                dst_abs, iter_temp_file(temp_path), overwrite=overwrite
            )
...
async def process_file(
    ...
    processor = processor_service.get_processor(processor_type)
    if not processor:
    ...
    allowed_exts = {
        str(ext).lower().lstrip(".") for ext in supported_exts if isinstance(ext, str)
    }
    ...
    def coerce_result_bytes(result: Any) -> bytes:
        ...
        raise HTTPException(
            500, detail="Processor must return bytes/Response when produces_file=True"
        )
    ...
    if actual_is_dir:
        if save_to:
            raise HTTPException(
                400, detail="Directory processing does not support custom save_to path"
            )
        if not overwrite:
        ...
        while stack:
            ...
            while True:
                entries, total = await list_dir(
                    root, current, page, page_size, "name", "asc"
                )
                if not entries and (total or 0) == 0:
                    break
...
async def get_temp_link_secret_key() -> bytes:
    ...
    return await config_service.get_secret_key("TEMP_LINK_SECRET_KEY", None)
