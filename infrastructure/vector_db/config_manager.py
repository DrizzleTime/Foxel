...
class VectorDBConfigManager:
    ...
    async def load_config(cls) -> Tuple[str, Dict[str, Any]]:
        raw_type = await config_service.get(cls.TYPE_KEY, cls.DEFAULT_TYPE)
        provider_type = str(raw_type or cls.DEFAULT_TYPE)

        raw_config = await config_service.get(cls.CONFIG_KEY)
    ...
    async def save_config(cls, provider_type: str, config: Dict[str, Any]) -> None:
        await config_service.set(cls.TYPE_KEY, provider_type)
        await config_service.set(cls.CONFIG_KEY, json.dumps(config or {}))
