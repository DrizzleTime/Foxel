...
# 支持的视频扩展名
VIDEO_EXT = {
    "mp4", "webm", "ogg", "m4v", "mov", "mkv", "avi", "wmv", "flv", "3gp",
    "m2ts", "ts", "mts", "vob", "mpg", "mpeg", "m4p", "m4b", "asf", "rm",
    "rmvb", "divx", "f4v", "ogv", "3g2", "dv", "swf",
}
...
def _parse_season_episode(rel_path: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    if m:
        ...
        for part in reversed(Path(rel_path).parts[:-1]):
            sm = (
                _CN_SEASON_RE.search(part)
                or _SEASON_WORD_RE.search(part)
                or _S_RE.search(part)
            )
            if sm:
    ...
    async def get(self, path: str, params: dict) -> dict:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await client.get(
                url, headers=self._headers(), params=self._merge_params(params)
            )
...
class VideoLibraryProcessor:
    ...
    async def process(
        self, input_bytes: bytes, path: str, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        # 延迟导入以避免插件加载时的循环依赖
        ...
        if is_dir:
            payload, saved_to = await self._process_tv_dir(
                tmdb, path, query_name, language, scraped_at, config
            )
            return {
                "ok": True,
                "type": "tv",
        ...
        payload, saved_to = await self._process_movie_file(
            tmdb, path, query_name, language, scraped_at
        )
        return {
            "ok": True,
            "type": "movie",
    ...
    async def _process_movie_file(
        ...
        search = await tmdb.get(
            "/search/movie", {"query": query_name, "language": language}
        )
    ...
    async def _process_tv_dir(
        ...
        search = await tmdb.get(
            "/search/tv", {"query": query_name, "language": language}
        )
        results = search.get("results") or []
        if not results:
        ...
        if include_episodes:
            ...
            seasons = sorted(
                {ep["season"] for ep in episodes if ep.get("season") is not None}
            )
    ...
    async def _collect_episode_files(self, dir_path: str) -> List[dict]:
        ...
        adapter_instance, adapter_model, root, rel = (
            await VirtualFSService.resolve_adapter_and_rel(dir_path)
        )
        rel = rel.rstrip("/")
        ...
        while stack:
            ...
            while True:
                ...
                for entry in entries:
                    ...
                    absolute_path = VirtualFSService._build_absolute_path(
                        adapter_model.path, child_rel
                    )
    ...
    def _attach_tmdb_episode_detail(
        self, episodes: List[dict], seasons_detail: Dict[str, Any]
    ) -> None:
